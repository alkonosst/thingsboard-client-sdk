...
// Log messages.
#if THINGSBOARD_ENABLE_PROGMEM
constexpr char UNABLE_TO_DE_SERIALIZE_JSON[] PROGMEM =
  "Unable to de-serialize received json data with error (DeserializationError::%s)";
constexpr char INVALID_BUFFER_SIZE[] PROGMEM =
  "Buffer size (%u) to small for the given payloads size (%u), increase with setBufferSize "
  "accordingly or set THINGSBOARD_ENABLE_STREAM_UTILS to 1 before including ThingsBoard";
#if THINGSBOARD_ENABLE_OTA
constexpr char NUMBER_PRINTF[] PROGMEM = "%u";
#endif // THINGSBOARD_ENABLE_OTA
#if !THINGSBOARD_ENABLE_DYNAMIC
constexpr char MAX_RPC_EXCEEDED[] PROGMEM =
  "Too many server-side RPC subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_RPC_REQUEST_EXCEEDED[] PROGMEM =
  "Too many client-side RPC subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_SHARED_ATT_UPDATE_EXCEEDED[] PROGMEM =
  "Too many shared attribute update callback subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_SHARED_ATT_REQUEST_EXCEEDED[] PROGMEM =
...
constexpr char ATT_CB_NO_KEYS[] PROGMEM =
  "No keys subscribed. Calling subscribed callback for any updated attributes, assumed to be "
  "subscribed to every possible key";
constexpr char ATT_IS_NULL[] PROGMEM = "Subscribed shared attribute update key is NULL";
constexpr char ATT_NO_CHANGE[] PROGMEM =
  "No keys that we subscribed too were changed, skipping callback";
constexpr char CALLING_RPC_CB[] PROGMEM =
  "Calling subscribed callback for rpc with methodname (%s)";
constexpr char CALLING_ATT_CB[] PROGMEM =
  "Calling subscribed callback for updated shared attribute (%s)";
constexpr char CALLING_REQUEST_CB[] PROGMEM =
  "Calling subscribed callback for request with response id (%u)";
...
#endif // THINGSBOARD_ENABLE_OTA
#if !THINGSBOARD_ENABLE_DYNAMIC
constexpr char MAX_RPC_EXCEEDED[] =
  "Too many server-side RPC subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_RPC_REQUEST_EXCEEDED[] =
  "Too many client-side RPC subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_SHARED_ATT_UPDATE_EXCEEDED[] =
  "Too many shared attribute update callback subscriptions, increase MaxFieldsAmt or unsubscribe";
constexpr char MAX_SHARED_ATT_REQUEST_EXCEEDED[] =
  "Too many shared attribute request callback subscriptions, increase MaxFieldsAmt";
#else
...
constexpr char ATT_CB_NO_KEYS[] = "No keys subscribed. Calling subscribed callback for any updated "
                                  "attributes, assumed to be subscribed to every possible key";
...
constexpr char CALLING_REQUEST_CB[] =
  "Calling subscribed callback for request with response id (%u)";
...
constexpr char SEND_SERIALIZED[] =
  "Hidden, because json data is bigger than buffer, therefore showing in console is skipped";
#endif // THINGSBOARD_ENABLE_DEBUG
#endif // THINGSBOARD_ENABLE_PROGMEM
...
constexpr char NOT_ENOUGH_RAM[] PROGMEM =
  "Temporary allocating more internal client buffer failed, decrease OTA chunk size or decrease "
  "overall heap usage";
constexpr char RESETTING_FAILED[] PROGMEM =
...
constexpr char NOT_ENOUGH_RAM[] = "Temporary allocating more internal client buffer failed, "
                                  "decrease OTA chunk size or decrease overall heap usage";
constexpr char RESETTING_FAILED[] =
  "Preparing for OTA firmware updates failed, attributes might be NULL";
#if THINGSBOARD_ENABLE_DEBUG
...
#if THINGSBOARD_ENABLE_DYNAMIC
/// @brief Wrapper around any arbitrary MQTT Client implementing the IMQTT_Client interface, to
/// allow connecting and sending / retrieving data from ThingsBoard over the MQTT or MQTT with
/// TLS/SSL protocol. BufferSize of the underlying data buffer can be changed during the runtime and
/// the maximum amount of data points that can ever be sent or received are automatically deduced at
/// runtime If this feature of automatic deduction, is not needed, or not wanted because it
/// allocates memory on the heap, then the values can be set once as template arguements. Simply set
/// THINGSBOARD_ENABLE_DYNAMIC to 0, before including ThingsBoard.h
/// @tparam Logger Logging class that should be used to print messages generated by internal
/// processes, default = ThingsBoardDefaultLogger
template <typename Logger = ThingsBoardDefaultLogger>
#else
/// @brief Wrapper around any arbitrary MQTT Client implementing the IMQTT_Client interface, to
/// allow connecting and sending / retrieving data from ThingsBoard over the MQTT or MQTT with
/// TLS/SSL protocol. BufferSize of the underlying data buffer can be changed during the runtime and
/// the maximum amount of data points that can ever be can be set once as template arguements.
/// Changing is only possible if a new instance of this class is created. If these values should be
/// automatically deduced at runtime instead then, simply set THINGSBOARD_ENABLE_DYNAMIC to 1,
/// before including ThingsBoard.h
/// @tparam MaxFieldsAmt Maximum amount of key value pair that we will be able to sent or received
/// by ThingsBoard in one call, default = 8
/// @tparam Logger Logging class that should be used to print messages generated by internal
/// processes, default = ThingsBoardDefaultLogger
template <size_t MaxFieldsAmt = Default_Fields_Amt, typename Logger = ThingsBoardDefaultLogger>
#endif // THINGSBOARD_ENABLE_DYNAMIC
class ThingsBoardSized {
  public:
  /// @brief Constructs a ThingsBoardSized instance with the given network client that should be
  /// used to establish the connection to ThingsBoard
  /// @param client MQTT Client implementation that should be used to establish the connection to
  /// ThingsBoard
  /// @param bufferSize Maximum amount of data that can be either received or sent to ThingsBoard at
  /// once, if bigger packets are received they are discarded and if we attempt to send data that is
  /// bigger, it will not be sent, the internal value can be changed later at any time with the
  /// setBufferSize() method alternatively setting THINGSBOARD_ENABLE_STREAM_UTILS to 1 allows to
  /// send arbitrary size payloads if that is done the internal buffer of the MQTT Client
  /// implementation can be theoretically set to only be as big as the biggest message we should
  /// every receive from ThingsBoard, this will mean though that all messages are sent over the
  /// StreamUtils library as long as they are bigger than the internal buffer, which needs more time
  /// than sending a message directly but has the advantage of requiring less memory. So if that is
  /// a problem on the board it might be useful to enable the THINGSBOARD_ENABLE_STREAM_UTILS option
  /// and decrease the internal buffer size of the mqtt client to what is needed to receive all MQTT
  /// messages, that size can vary but if all ThingsBoard features are used a buffer size of 256
  /// bytes should suffice for receiving most responses. If the aforementioned feature is not
  /// enabled the buffer size might need to be much bigger though, but in that case if a message was
  /// too big to be sent the user will be informed with a message to the Logger. The aforementioned
  /// options can only be enabled if Arduino is used to build this library, because the StreamUtils
  /// library requires it, default = Default_Payload
  /// @param maxStackSize Maximum amount of bytes we want to allocate on the stack, default =
  ...
  /// but it can not do that anymore, because it has been disconnected or certain settings were
  /// changed
  /// @return Reference to the underlying MQTT Client implementation connected to ThingsBoard
  inline IMQTT_Client& getClient() { return m_client; }

  /// @brief Sets the maximum amount of bytes that we want to allocate on the stack, before the
  /// memory is allocated on the heap instead
  /// @param maxStackSize Maximum amount of bytes we want to allocate on the stack
  inline void setMaximumStackSize(const size_t& maxStackSize) { m_max_stack = maxStackSize; }

#if THINGSBOARD_ENABLE_STREAM_UTILS

  /// @brief Sets the amount of bytes that can be allocated to speed up fall back serialization with
  /// the StreamUtils class See https://github.com/bblanchon/ArduinoStreamUtils for more information
  /// on the underlying class used
  /// @param bufferingSize Amount of bytes allocated to speed up serialization
  inline void setBufferingSize(const size_t& bufferingSize) { m_buffering_size = bufferingSize; }

#endif // THINGSBOARD_ENABLE_STREAM_UTILS

  /// @brief Sets the size of the buffer for the underlying network client that will be used to
  /// establish the connection to ThingsBoard
  /// @param bufferSize Maximum amount of data that can be either received or sent to ThingsBoard at
  /// once, if bigger packets are received they are discarded and if we attempt to send data that is
  /// bigger, it will not be sent, the internal value can be changed later at any time with the
  /// setBufferSize() method alternatively setting THINGSBOARD_ENABLE_STREAM_UTILS to 1 allows to
  ...
  /// therefore there is no need anymore to discard all previously subscribed callbacks and letting
  /// the user resubscribe
  ...
  /// @param access_token Access token that connects this device with a created device on the
  /// ThingsBoard server, can be "provision", if the device creates itself instead. See
  /// https://thingsboard.io/docs/user-guide/device-provisioning/?mqttprovisioning=without#provision-device-apis
  /// for more information
  /// @param port Port that will be used to establish a connection and send / receive data from
  /// ThingsBoard over
  /// @param client_id Client username that can be used to differentiate the user that is connecting
  /// the given device to ThingsBoard
  /// @param password Client password that can be used to authenticate the user that is connecting
  /// the given device to ThingsBoard
  /// @return Whether connecting to ThingsBoard was successful or not
  inline bool connect(const char* host, const char* access_token = PROV_ACCESS_TOKEN,
                      const uint16_t& port = 1883, const char* client_id = DEFAULT_CLIENT_ID,
  ...
  inline bool Send_Json(const char* topic, const TSource& source, const size_t& jsonSize) {
    ...
    // if it did the isNull() method will return true. See
    // https://arduinojson.org/v6/api/jsonvariant/isnull/ for more information
    if (source.isNull()) {
    ...
    // if it is utilize the serialize json work around, so that the internal client buffer can be
    // circumvented
    ...
    // if it would allocate the memory on the heap instead to ensure no stack overflow occurs
      ...
      if (getMaximumStackSize() < jsonSize) {
        ...
        // Ensure to actually delete the memory placed onto the heap, to make sure we do not create
        // a memory leak and set the pointer to null so we do not have a dangling reference.
        delete[] json;
        json = nullptr;
      } else {
        char json[jsonSize];
...
#if THINGSBOARD_ENABLE_DEBUG
    char
      message[JSON_STRING_SIZE(strlen(SEND_MESSAGE)) + JSON_STRING_SIZE(strlen(topic)) + jsonSize];
  ...
  /// @brief Sends a claiming request for the given device, allowing any given user on the cloud to
  /// assign the device as their own (claim), as long as they enter the given device name and secret
  /// key in the given amount of time. Optionally a secret key can be passed or be left empty (cloud
  /// will allow any user to claim the device for the given amount of time). See
  /// https://thingsboard.io/docs/user-guide/claiming-devices/ for more information
  /// @param secretKey Password the user additionaly to the device name needs to enter to claim it
  /// as their own, pass nullptr or an empty string if the user should be able to claim the device
  ...
  /// @brief Sends provisioning request for a new device, meaning we want to create a device that we
  /// can then connect over, where the given provision device key / secret decide which device
  /// profile is used to create the given device with. Optionally a device name can be passed or be
  /// left empty (cloud will use a random string as the name instead). The cloud then sends back
  /// json data containing our credentials, that will call the given callback, if creating the
  /// device was successful. The data contained in that callbackcan then be used to disconnect and
  /// reconnect to the ThingsBoard server as our newly created device. See
  /// https://thingsboard.io/docs/user-guide/device-provisioning/ for more information
  /// @param callback Callback method that will be called upon data arrival with the given data that
  /// was received serialized into a JsonDocument
  ...
  inline bool Provision_Request(const Provision_Callback& callback) {
    ...
    // meaning the rest will not be sent and therefore the provisioning request has the correct
    // formatting
  ...
  /// that will be called if the key-value pair from the server for the given client-side attributes
  /// is received. See
  /// https://thingsboard.io/docs/reference/mqtt-api/#request-attribute-values-from-the-server for
  /// more information
  ...
  /// that will be called if a request from the server for the method with the given name is
  /// received. See https://thingsboard.io/docs/user-guide/rpc/#server-side-rpc for more information
  ...
  /// @param callbacksSize Amount of values that should be subscribed, ensure size matches the
  /// actual array, if not the system might crash unexpectedly at a later point
  /// @return Whether subscribing the given callbacks was successful or not
  ...
  /// that will be called if a response from the server for the method with the given name is
  /// received. See https://thingsboard.io/docs/user-guide/rpc/#client-side-rpc for more information
  /// @param callback Callback method that will be called
  /// @return Whether requesting the given callback was successful or not
...
#if THINGSBOARD_ENABLE_DYNAMIC
    // String are const char* and therefore stored as a pointer --> zero copy, meaning the size for
    // the strings is 0 bytes, Data structure size depends on the amount of key value pairs passed +
    // the default methodName and params key needed for the request. See
    // https://arduinojson.org/v6/assistant/ for more information on the needed size for the
    // JsonDocument
    const size_t dataStructureMemoryUsage =
      JSON_OBJECT_SIZE(parameters != nullptr ? parameters->size() + 2U : 2U);
    TBJsonDocument requestBuffer(dataStructureMemoryUsage);
#else
    // Ensure to have enough size for the infinite amount of possible parameters that could be sent
    // to the cloud, therefore we set the size to the MaxFieldsAmt instead of JSON_OBJECT_SIZE(1),
    // which will result in a JsonDocument with a size of 16 bytes
  ...
  inline bool Start_Firmware_Update(const OTA_Update_Callback& callback) {
    ...
    const std::vector<const char*> fw_shared_keys{
      FW_CHKS_KEY, FW_CHKS_ALGO_KEY, FW_SIZE_KEY, FW_TITLE_KEY, FW_VER_KEY};
    const Attribute_Request_Callback fw_request_callback(
      std::bind(&ThingsBoardSized::Firmware_Shared_Attribute_Received, this, std::placeholders::_1),
      fw_shared_keys.cbegin(),
      fw_shared_keys.cend());
    return Shared_Attributes_Request(fw_request_callback);
  }

  /// @brief Stops the currently running firmware update, calls the finish callback with a failure
  /// if the update is running. See https://thingsboard.io/docs/user-guide/ota-updates/ for more
  /// information
  inline void Stop_Firmware_Update() { m_ota.Stop_Firmware_Update(); }
  ...
  inline bool Subscribe_Firmware_Update(const OTA_Update_Callback& callback) {
    ...
    const std::vector<const char*> fw_shared_keys{
      FW_CHKS_KEY, FW_CHKS_ALGO_KEY, FW_SIZE_KEY, FW_TITLE_KEY, FW_VER_KEY};
    const Shared_Attribute_Callback fw_update_callback(
      std::bind(&ThingsBoardSized::Firmware_Shared_Attribute_Received, this, std::placeholders::_1),
      fw_shared_keys.cbegin(),
      fw_shared_keys.cend());
  ...
  inline bool Firmware_Send_Info(const char* currFwTitle, const char* currFwVersion) {
    ...
    return sendTelemetryJson(currentFirmwareInfoObject,
                             Helper::Measure_Json(currentFirmwareInfoObject));
  }
  ...
  inline bool Firmware_Send_State(const char* currFwState, const char* fwError = nullptr) {
    ...
    return sendTelemetryJson(currentFirmwareStateObject,
                             Helper::Measure_Json(currentFirmwareStateObject));
  }
  ...
  /// that will be called if the key-value pair from the server for the given shared attributes is
  /// received. See
  /// https://thingsboard.io/docs/reference/mqtt-api/#request-attribute-values-from-the-server for
  /// more information
  /// @param callback Callback method that will be called
  ...
  /// @return Whether subscribing the given callbacks was successful or not
  template <class InputIterator>
  inline bool Shared_Attributes_Subscribe(const InputIterator& first_itr,
                                          const InputIterator& last_itr) {
#if !THINGSBOARD_ENABLE_DYNAMIC
    const size_t size = std::distance(first_itr, last_itr);
    ...
    m_shared_attribute_update_callbacks.insert(
      m_shared_attribute_update_callbacks.end(), first_itr, last_itr);
    return true;
  }
  ...
  /// actual array, if not the system might crash unexpectedly at a later point
  /// @return Whether subscribing the given callbacks was successful or not
  inline bool Shared_Attributes_Subscribe(const Shared_Attribute_Callback* callbacks,
  ...
  /// Sends the given bytes to the client without requiring any temporary buffer at the cost of
  /// hugely increased send times
  /// @tparam TSource Source class that should be used to serialize the json that is sent to the
  /// server
  /// @param topic Topic we want to send the data over
  /// @param source Data source containing our json key value pairs we want to send
  ...
  inline bool Publish_Chunk_Request(const size_t& request_chunck) {
    ...
    // Size adjuts dynamically to the current length of the currChunk number to ensure we don't cut
    // it out of the topic string.
  ...
  /// @brief Returns the maximum amount of bytes that we want to allocate on the stack, before the
  /// memory is allocated on the heap instead
  /// @return Maximum amount of bytes we want to allocate on the stack
  inline const size_t& getMaximumStackSize() const { return m_max_stack; }

#if THINGSBOARD_ENABLE_STREAM_UTILS

  /// @brief Returns the amount of bytes that can be allocated to speed up fall back serialization
  /// with the StreamUtils class See https://github.com/bblanchon/ArduinoStreamUtils for more
  /// information on the underlying class used
  /// @return Amount of bytes allocated to speed up serialization
  inline const size_t& getBufferingSize() const { return m_buffering_size; }

#endif // THINGSBOARD_ENABLE_STREAM_UTILS

  /// @brief Requests one client-side or shared attribute calllback,
  /// that will be called if the key-value pair from the server for the given client-side or shared
  /// attributes is received
  /// @param callback Callback method that will be called
  /// @param attributeRequestKey Key of the key-value pair that will contain the attributes we want
  /// to request
  /// @param attributeResponseKey Key of the key-value pair that will contain the attributes we got
  /// as a response
  /// @return Whether requesting the given callback was successful or not
  inline bool Attributes_Request(const Attribute_Request_Callback& callback,
    ...
    // String are const char* and therefore stored as a pointer --> zero copy, meaning the size for
    // the strings is 0 bytes, Data structure size depends on the amount of key value pairs passed +
    // the default clientKeys or sharedKeys See https://arduinojson.org/v6/assistant/ for more
    // information on the needed size for the JsonDocument
  ...
  /// @param callback Callback method that will be called
  /// @return Whether checking and sending the current device firmware information was successful or
  /// not
  ...
  /// @brief Unsubscribes from the firmware response topic and clears any memory associated with the
  /// firmware update, should not be called before actually fully completing the firmware update.
  ...
  inline bool Firmware_OTA_Unsubscribe() {
    ...
    // buffer size would allow, therefore we return to the previous value to decrease overall memory
    // usage
  ...
  inline void Firmware_Shared_Attribute_Received(const Shared_Attribute_Data& data) {
    ...
    // If firmware version and title is the same, we do not initiate an update, because we expect
    // the binary to be the same one we are currently using
    else if (strncmp_P(curr_fw_title, fw_title, JSON_STRING_SIZE(strlen(curr_fw_title))) == 0 &&
             strncmp_P(curr_fw_version, fw_version, JSON_STRING_SIZE(strlen(curr_fw_version))) ==
               0) {
      Logger::log(FW_UP_TO_DATE);
      Firmware_Send_State(FW_STATE_FAILED, FW_UP_TO_DATE);
      return;
    }
    // If firmware title is not the same, we do not initiate an update, because we expect the binary
    // to be for another device type
    else if (strncmp_P(curr_fw_title, fw_title, JSON_STRING_SIZE(strlen(curr_fw_title))) != 0) {
    ...
    // Change the used firmware algorithm, depending on which type is set for the given firmware
    // information
    ...
    m_ota.Start_Firmware_Update(
      m_fw_callback, fw_size, fw_algorithm, fw_checksum, fw_checksum_algorithm);
  }

#endif // THINGSBOARD_ENABLE_OTA

  /// @brief Connects to the previously set ThingsBoard server, as the given client with the given
  /// access token
  /// @param access_token Access token that connects this device with a created device on the
  /// ThingsBoard server, can be "provision", if the device creates itself instead
  /// @param client_id Client username that can be used to differentiate the user that is connecting
  /// the given device to ThingsBoard
  /// @param password Client password that can be used to authenticate the user that is connecting
  /// the given device to ThingsBoard
  /// @return Whether connecting to ThingsBoard was successful or not
  inline bool connect_to_host(const char* access_token, const char* client_id,
                              const char* password) {
  ...
  /// @brief Resubscribes to topics that establish a permanent connection with MQTT, meaning they
  /// may receive more than one event over their lifetime, whereas other events that are only ever
  /// called once and then deleted after they have been handled are not resubscribed. This is done,
  /// because the chance of disconnecting the moment when a request event (provisioning, attribute
  /// request, client-side rpc) was sent and then reconnecting and resubscribing to that topic fast
  /// enough to still receive the message is not feasible
...
#if !THINGSBOARD_ENABLE_DYNAMIC
  /// @brief Reserves size for the given amount of items in our internal callback vectors beforehand
  /// for performance reasons, this ensures the internal memory blocks do not have to move if new
  /// data is inserted, because the current memory block is too small to hold the data for all the
  /// values
  ...
  /// @param callback Callback method that will be called
  /// @param registeredCallback Editable pointer to a reference of the local version that was copied
  /// from the passed callback
  /// @return Whether requesting the given callback was successful or not
  inline bool RPC_Request_Subscribe(const RPC_Request_Callback& callback,
                                    RPC_Request_Callback*& registeredCallback = nullptr) {
#if !THINGSBOARD_ENABLE_DYNAMIC
  ...
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @param telemetry Whether the data we want to send should be sent as an attribute or telemetry
  /// data value
  /// @return Whether sending the data was successful or not
  template <typename T>
  inline bool sendKeyValue(const char* key, T value, bool telemetry = true) {
    ...
    return telemetry ? sendTelemetryJson(object, Helper::Measure_Json(object))
                     : sendAttributeJSON(object, Helper::Measure_Json(object));
  }

  /// @brief Process callback that will be called upon client-side RPC response arrival
  /// and is responsible for handling the payload and calling the appropriate previously subscribed
  /// callbacks
  ...
  inline void process_rpc_request_message(char* topic, const JsonObjectConst& data) {
    // Remove the not needed part of the received topic string, which is everything before the
    // response id, therefore we remove the section before that which is the topic + an additional
    // "/" character, that seperates the topic from the response id. Meaning the index we want to
    // get the substring from is the length of the topic + 1 for the additonal "/" character
    ...
    // Convert the remaining text after the topic to an integer, because it should now contain only
    // the response id
    ...
    for (size_t i = 0; i < m_rpc_request_callbacks.size(); i++) {
      ...
      // Delete callback because the changes have been requested and the callback is no longer
      // needed
  ...
  inline void process_rpc_message(char* topic, const JsonObjectConst& data) {
    ...
    for (const RPC_Callback& rpc : m_rpc_callbacks) {
      ...
      // meaning 0 is the same string and less and more than 0 is the difference in ascci values
      // between the 2 chararacters.
    ...
    // Remove the not needed part of the received topic string, which is everything before the
    // request id, therefore we remove the section before that which is the topic + an additional
    // "/" character, that seperates the topic from the request id. Meaning the index we want to get
    // the substring from is the length of the topic + 1 for the additonal "/" character
  ...
  inline void process_firmware_response(char* topic, uint8_t* payload, const size_t& length) {
    ...
    if (getMaximumStackSize() < length) {
      ...
      // Ensure to actually delete the memory placed onto the heap, to make sure we do not create a
      // memory leak and set the pointer to null so we do not have a dangling reference.
      delete[] binary;
      binary = nullptr;
    } else {
...
#if THINGSBOARD_ENABLE_DEBUG
      char calling_message[JSON_STRING_SIZE(strlen(CALLING_ATT_CB)) +
                           JSON_STRING_SIZE(strlen(requested_att))];
  ...
  /// @brief Process callback that will be called upon client-side or shared attribute request
  /// arrival and is responsible for handling the payload and calling the appropriate previously
  ...
  inline bool sendDataArray(const Telemetry* data, size_t data_count, bool telemetry = true) {
    ...
    // String are const char* and therefore stored as a pointer --> zero copy, meaning the size for
    // the strings is 0 bytes, Data structure size depends on the amount of key value pairs passed.
    // See https://arduinojson.org/v6/assistant/ for more information on the needed size for the
    // JsonDocument
  ...
  // This can be done because all Callback methods mostly consists of pointers to actual object so
  // copying them does not require a huge memory overhead and is acceptable especially in comparsion
  // to possible problems that could arise if references were used and the end user does not take
  // care to ensure the Callbacks live on for the entirety of its usage, which will lead to dangling
  // references and undefined behaviour. Therefore copy-by-value has been choosen as for this
  // specific use case it is more advantageous, especially because at most we copy a vector, that
  // will only ever contain a few pointers
  Vector<RPC_Callback>
    m_rpc_callbacks; // Server side RPC callbacks vector, replacement for non C++ STL boards
  Vector<RPC_Request_Callback>
    m_rpc_request_callbacks; // Client side RPC callbacks vector, replacement for non C++ STL boards
  Vector<Shared_Attribute_Callback>
    m_shared_attribute_update_callbacks; // Shared attribute update callbacks vector, replacement
                                         // for non C++ STL boards
  Vector<Attribute_Request_Callback>
  ...
  uint16_t m_previous_buffer_size; // Previous buffer size of the underlying client, used to revert
                                   // to the previously configured buffer size if it was temporarily
                                   // increased by the OTA update
  bool m_change_buffer_size; // Whether the buffer size had to be changed, because the previous
                             // internal buffer size was to small to hold the firmware chunks
  OTA_Handler<Logger> m_ota; // Class instance that handles the flashing and creating a hash from
                             // the given received binary firmware data
#endif                       // THINGSBOARD_ENABLE_OTA
  ...
  inline void onMQTTMessage(char* topic, uint8_t* payload, unsigned int length) {
...
#if THINGSBOARD_ENABLE_OTA
    // When receiving the ota binary payload we do not want to deserialize it into json, because it
    // only contains firmware bytes that should be directly writtin into flash, therefore we can
    // skip that step and directly process those bytes
...
#if THINGSBOARD_ENABLE_DYNAMIC
    // Buffer that we deserialize is writeable and not read only --> zero copy, meaning the size for
    // the data is 0 bytes, Data structure size depends on the amount of key value pairs received.
    // See https://arduinojson.org/v6/assistant/ for more information on the needed size for the
    // JsonDocument
    const size_t dataStructureMemoryUsage =
      JSON_OBJECT_SIZE(Helper::getOccurences(reinterpret_cast<char*>(payload), COLON));
    TBJsonDocument jsonBuffer(dataStructureMemoryUsage);
#else
    StaticJsonDocument<JSON_OBJECT_SIZE(MaxFieldsAmt)> jsonBuffer;
#endif // !THINGSBOARD_ENABLE_DYNAMIC

    // The deserializeJson method we use, can use the zero copy mode because a writeable input was
    // passed, if that were not the case the needed allocated memory would drastically increase,
    // because the keys would need to be copied as well. See
    // https://arduinojson.org/v6/doc/deserialization/ for more info on ArduinoJson deserialization
    ...
    // .as() is used instead of .to(), because it is meant to cast the JsonDocument to the given
    // type, but it does not change the actual content of the JsonDocument, we don't want that
    // because it already contents content and would result in the data simply being "null", instead
    // .as() allows accessing the data over a JsonObjectConst instead.
    ...
    // that is the case because the more specific one that is longer but contains the same text has
    // to be checked first, because if we do not do that then even if we receive a message from the
    // ATTRIBUTE_RESPONSE_TOPIC we would call the process method for the ATTRIBUTE_TOPIC because we
    // only compare until the end of the ATTRIBUTE_TOPIC string, therefore the received topic is
    // exactly the same. Therefore the ordering needs to stay the same for thoose two specific
    // checks
...
ThingsBoardSized<MaxFieldsAmt, Logger>*
  ThingsBoardSized<MaxFieldsAmt, Logger>::m_subscribedInstance = nullptr;
